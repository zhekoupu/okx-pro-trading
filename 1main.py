...
class SignalChecker:
    ...
    def _detect_doji(self, data: pd.DataFrame) -> bool:
        ...
        return body / total_range <= UltimateConfig.CANDLE_PATTERNS['doji_body_ratio']
...
class BacktestEngine:
    def __init__(self, signal_checker):
        self.signal_checker = signal_checker
        self.results = []
        
    def run_backtest(self, coins_data, start_date=None, end_date=None):
        """运行回测"""
        print("\n📊 开始回测...")
        ...
        for symbol, data_dict in coins_data.items():
            for tf, df in data_dict.items():
                ...
                for i in range(50, len(df)):
                    ...
                    # 检查信号
                    signals = self.signal_checker.check_all_coins({symbol: historical_data}, None)
    ...
    def _simulate_trade(self, signal, future_data):
        """模拟单笔交易"""
        entry_price = signal['entry_points']['main_entry']
        stop_loss = signal['entry_points']['stop_loss']
        take_profit = signal['entry_points']['take_profit2']
    ...
    def _analyze_results(self):
        ...
        for pattern in df['pattern'].unique():
            ...
            print(f"\n   {pattern}:")
            print(f"     次数: {len(pattern_df)}")
            print(f"     胜率: {(pattern_df['profit_pct'] > 0).mean() * 100:.1f}%")
            print(f"     平均收益: {pattern_df['profit_pct'].mean():.2f}%")
            
        # 保存结果
        with open(UltimateConfig.BACKTEST_RESULTS_FILE, 'w') as f:
            json.dump([r.__dict__ for r in self.results], f, indent=2, cls=DateTimeEncoder)
            
        print(f"\n✅ 回测结果已保存到 {UltimateConfig.BACKTEST_RESULTS_FILE}")
...
class UltimateTradingSystem:
    def __init__(self):
        ...
        self.backtest_engine = BacktestEngine(self.signal_checker) if BACKTEST_MODE else None
    ...
    def run_backtest(self, days=30):
        ...
        # 这里简化处理，实际应该分批获取历史数据
        coins_data = self.data_fetcher.get_all_coins_data(MONITOR_COINS[:10])  # 只测前10个币种
        
        if coins_data:
            self.backtest_engine.run_backtest(coins_data)
...
def main():
    ...
    try:
        ...
        # 检查是否回测模式
        if BACKTEST_MODE and len(sys.argv) > 1 and sys.argv[1] == 'backtest':
