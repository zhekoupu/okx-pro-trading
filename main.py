...
14. 系统层面优化：ATR最小止损保护、小币种仓位衰减、RSI分级加权
"""
...
COIN_GROUPS = {
    ...
    'DEFI': ['UNI','LINK','AAVE','COMP','CRV','TRB','DYDX','SUSHI','TIA','PENGU'],
    'GAMING': ['SAND','MANA','ENJ','GALA','AXS','LTC','XAU'],
...
class UltimateConfig:
    ...
    # ===== 止损最小距离（基于ATR）=====
    MIN_STOP_ATR_MULTIPLIER = 0.8        # 最小止损距离为ATR的倍数
    
    # ===== 小币种仓位衰减 =====
    # 使用板块来判断小市值：这些板块的币种视为小市值
    SMALL_CAP_GROUPS = ['MEME', 'AI', 'RWA', 'STORAGE', 'PRIVACY', 'OTHERS']
    SMALL_CAP_DECAY_FACTOR = 0.7
...
class SignalChecker:
    ...
    def _calculate_position_size(self, score: int, data: pd.DataFrame, price: float, symbol: str) -> float:
        """计算仓位大小，包含小币种衰减"""
        ...
        # 小币种仓位衰减
        group = COIN_TO_GROUP.get(symbol, 'OTHERS')
        if group in UltimateConfig.SMALL_CAP_GROUPS:
            base_size *= UltimateConfig.SMALL_CAP_DECAY_FACTOR
    ...
    def _calculate_stop_loss(self, data: pd.DataFrame, price: float, direction: str,
        ...
        if direction == 'BUY':
            ...
            # ATR最小止损距离保护
            min_stop_distance = atr * UltimateConfig.MIN_STOP_ATR_MULTIPLIER
            if (entry_main - stop_loss) < min_stop_distance:
                stop_loss = entry_main - min_stop_distance
            ...
            # ATR最小止损距离保护
            min_stop_distance = atr * UltimateConfig.MIN_STOP_ATR_MULTIPLIER
            if (stop_loss - entry_main) < min_stop_distance:
                stop_loss = entry_main + min_stop_distance
    ...
    def _calculate_bounce_score(self, rsi, volume_ratio):
        ...
        # RSI分级加权（超卖越严重加分越多）
        if rsi < 24:
            score += UltimateConfig.BOUNCE_RSI_BONUS[24]
        elif rsi < 28:
            score += UltimateConfig.BOUNCE_RSI_BONUS[28]
    ...
    def _create_bounce_signal(self, symbol, data, price, rsi, volume_ratio, ma20, score,
        ...
        position_size = self._calculate_position_size(score, data, price, symbol)
                    ...
                    if (direction == 'BUY' and current_trend_dir == 1) or \
                       (direction == 'SELL' and current_trend_dir == -1):
                        ...
                        if new_score >= UltimateConfig.HIGH_CONFIDENCE_THRESHOLD:
                            ...
                            signal['reason'] += "\n• 延迟1根K线确认趋势后增强"
                            signal['position_size'] = self._calculate_position_size(new_score, data_15m, signal['current_price'], symbol)
...
def main():
    ...
    print(f"📊 监控币种: {len(MONITOR_COINS)}个")
    print(f"🎯 信号模式: 动态分层 + 结构确认 + 多周期背离 + 板块风控 + 双向衰竭 + ATR止损保护 + 小币种仓位衰减 + RSI分级加权")
    print("=" * 70)
